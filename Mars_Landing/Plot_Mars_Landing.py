...
class Plot_Mars_Landing:
    ...
    def animate(self, population:list):
        ...
        for individu in population:
            ...
            ani = animation.FuncAnimation(self._fig, func=self._update, fargs=[x, y, line],
                                        blit = False, interval=1, repeat = False)
                                        # blit = False, frames=len(x)*100, interval=1, repeat = False)
                                        # blit = False, interval = len(x)//10, repeat = False)
                                        # blit = False, interval = min(len(x)*10, 1000), repeat = False)
            plt.draw()
            plt.pause(.5)
